...
class Main(metaclass=LogBase):
        ...
        if mtk.preloader.init():
            ...
            if mtk.port.cdc.pid == 0x0003:
                ...
                if plt.runpayload(filename=stage1file):
                    ...
                    self.debug(f"Done sending stage2, size {hex(len(stage2data))}.")
                    mtk.config.set_gui_status(mtk.config.tr("Done sending stage 2"))
                    if verifystage2:
                        self.debug("Verifying stage2 data")
                        rdata = b""
                        mtk.port.usbwrite(pack(">I", 0xf00dd00d))
                        ...
                        if rdata != stage2data:
                            ...
                            self.debug("Stage2 verification passed.")
    ...
    def cmd_peek(self, mtk, addr, length, preloader, filename):
        ...
        if mtk is not None:
            if preloader is not None:
                if os.path.exists(preloader):
                    ...
                    if mtk.preloader.send_da(daaddr, len(dadata), 0x100, dadata):
                        self.debug(f"Sent preloader to {hex(daaddr)}, length {hex(len(dadata))}")
                        if mtk.preloader.jump_da(daaddr):
                            self.debug(f"Jumped to pl {hex(daaddr)}.")
                            time.sleep(2)
            ...
            if filename is None:
                ...
                wf.close()
                self.debug(f"Data from {hex(addr)} with size of {hex(length)} was written to " + filename)
    ...
    def run(self, parser):
        ...
        if cmd == "script":
            ...
            if mtk.preloader.init():
                ...
                if data is not None:
                    ...
                    with open(filename, 'wb') as wf:
                        ...
                        self.debug("Preloader dumped as: " + filename)
                rmtk.port.close()
            self.close()
        elif cmd == "dumpsram":
            if mtk.preloader.init():
            ...
            self.debug("Kamakiri / DA Bruteforce run")
            ...
            if mtk.preloader.init():
                if mtk.config.target_config["daa"]:
                    ...
                    if mtk is None:
                        ...
                        return
                self.debug("Connected to device, loading")
            else:
            ...
            if mtk.config.preloader_filename is not None:
                self.debug("Using custom preloader : " + mtk.config.preloader_filename)
                ...
                if mtk.preloader.send_da(daaddr, len(dadata), 0x100, dadata):
                    self.debug(f"Sent preloader to {hex(daaddr)}, length {hex(len(dadata))}")
                    if mtk.preloader.jump_da(daaddr):
                        self.debug(f"PL Jumped to daaddr {hex(daaddr)}.")
                        ...
                        if self.args.metamode is not None:
                            ...
                            if meta.init(metamode=self.args.metamode, display=False):
                                self.debug(f"Successfully set meta mode : {self.args.metamode}")
                            mtk.port.close()
                            self.close()
                            return
                        if self.args.startpartition is not None or self.args.offset is not None or self.args.length is not None:
                            ...
                            mtk.port.close()
                            time.sleep(3)
                            self.debug(f"Keep pressed power button to boot.")
                            self.close()
                            return

                        if self.args.startpartition is not None:
                            partition = self.args.startpartition
                            self.debug("Booting to : " + partition)
                        ...
                        if self.args.offset is not None and self.args.length is not None:
                            ...
                            if status != 0x0:
                                ...
                                self.debug("Jumping to partition ....")
                            return
                        mtk.port.close()
                        sys.exit(0)
            if mtk.preloader.send_da(plstageaddr, len(pldata), 0x100, pldata):
                self.debug(f"Sent stage2 to {hex(plstageaddr)}, length {hex(len(pldata))}")
                mtk.preloader.get_hw_sw_ver()
                if mtk.preloader.jump_da(plstageaddr):
                    self.debug(f"Jumped to stage2 at {hex(plstageaddr)}.")
                    if os.path.basename(plstage) == "pl.bin":
                        ack = unpack(">I", mtk.port.usbread(4))[0]
                        if ack == 0xB1B2B3B4:
                            self.debug("Successfully loaded stage2")
                        ...
                        self.debug("Successfully loaded stage2, dis- and reconnect usb cable")
                        time.sleep(2)
                        ack = unpack(">I", mtk.port.usbread(4))[0]
                        mtk.port.close()
                        return
            ...
            if mtk.preloader.init():
                self.debug("Getting target info...")
    ...
    def cmd_log(self, mtk, filename):
        if mtk.preloader.init():
            self.debug("Getting target logs...")
            try:
            ...
            if logs != b"":
                with open(filename, "wb") as wf:
                    wf.write(logs)
                    self.debug(f"Successfully wrote logs to \"{filename}\"")
            else:
                self.debug("No logs found.")
