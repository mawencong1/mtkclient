...
class DAloader(metaclass=LogBase):
    ...
    def setmetamode(self, porttype: str):
        ...
        if self.xflash:
            ...
            if self.da.set_meta(porttype):
                self.debug(f"Successfully set meta mode to {porttype}")
                return True
            else:
    ...
    def readflash(self, addr, length, filename, parttype, display=True, packetsize = 0x100000):
        return self.da.readflash(addr=addr, length=length, filename=filename, parttype=parttype, display=display, packetsize = packetsize)
