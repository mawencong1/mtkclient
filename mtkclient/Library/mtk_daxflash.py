...
class DAXFlash(metaclass=LogBase):
                ...
                try:
                    if self.xsend(pack("<I", len(emi))):
                        if self.send_param([emi]):
                            self.debug(f"DRAM setup passed.")
                            return True
                except Exception as err:
                ...
                if self.usbwrite(pkt1):
                    if self.usbwrite(param):
                        if self.send_data(da):
                            self.debug(f"Upload data was accepted. Jumping to stage 2...")
                            if timeout:
                                time.sleep(timeout)
                            status = -1
                            try:
    ...
    def formatflash(self, addr, length, storage=None,
        ...
        self.debug(f"Formatting addr {hex(addr)} with length {hex(length)}, please standby....")
            ...
            if status == 0:
                ...
                if self.send_param(param):
                    ...
                    if status == 0x40040005:  # STATUS_COMPLETE
                        ...
                        self.debug(f"Successsfully formatted addr {hex(addr)} with length {length}.")
                        return True
    ...
    def get_da_version(self):
        ...
        status = self.status()
        if status == 0:
            self.debug(f"DA-VERSION      : {data.decode('utf-8')}")
            return data
        else:
    ...
    def get_chip_id(self):
        ...
        status = self.status()
        if status == 0:
            self.debug("HW-CODE         : 0x%X", chipid.hw_code)
            self.debug("HWSUB-CODE      : 0x%X", chipid.hw_sub_code)
            self.debug("HW-VERSION      : 0x%X", chipid.hw_version)
            self.debug("SW-VERSION      : 0x%X", chipid.sw_version)
            self.debug("CHIP-EVOLUTION  : 0x%X", chipid.chip_evolution)
            return chipid
        else:
            self.error(f"Error on getting chip id: {self.eh.status(status)}")
        return None
    ...
    def get_emmc_info(self, display=True):
        ...
        if status == 0:
            ...
            if emmc.type != 0 and display:
                self.debug(f"EMMC FWVer:      {hex(emmc.fwver)}")
                try:
                    self.debug(f"EMMC ID:         {emmc.cid[3:9].decode('utf-8')}")
                except:
                    pass
                self.debug(f"EMMC CID:        {hexlify(emmc.cid).decode('utf-8')}")
                if self.config.hwparam is not None:
                    self.config.set_cid(emmc.cid)
                self.debug(f"EMMC Boot1 Size: {hex(emmc.boot1_size)}")
                self.debug(f"EMMC Boot2 Size: {hex(emmc.boot2_size)}")
                self.debug(f"EMMC GP1 Size:   {hex(emmc.gp1_size)}")
                self.debug(f"EMMC GP2 Size:   {hex(emmc.gp2_size)}")
                self.debug(f"EMMC GP3 Size:   {hex(emmc.gp3_size)}")
                self.debug(f"EMMC GP4 Size:   {hex(emmc.gp4_size)}")
                self.debug(f"EMMC RPMB Size:  {hex(emmc.rpmb_size)}")
                self.debug(f"EMMC USER Size:  {hex(emmc.user_size)}")
            return emmc
        else:
    ...
    def get_nand_info(self, display=True):
        ...
        if status == 0:
            ...
            if nand.type != 0:
                if display:
                    self.debug(f"NAND Pagesize:   {hex(nand.page_size)}")
                    self.debug(f"NAND Blocksize:  {hex(nand.block_size)}")
                    self.debug(f"NAND Sparesize:  {hex(nand.spare_size)}")
                    self.debug(f"NAND Total size: {hex(nand.total_size)}")
                    self.debug(f"NAND Avail:      {hex(nand.available_size)}")
                    self.debug(f"NAND ID:         {hexlify(nand.nand_id).decode('utf-8')}")
            return nand
        else:
    ...
    def get_nor_info(self, display=True):
        ...
        if status == 0:
            ...
            if nor.type != 0:
                if display:
                    self.debug(f"NOR Pagesize: {hex(nor.page_size)}")
                    self.debug(f"NOR Size:     {hex(nor.available_size)}")
            return nor
        else:
    ...
    def get_ufs_info(self, display=True):
        ...
        if status == 0:
            ...
            if ufs.type != 0:
                if display:
                    self.debug(f"UFS FWVer:    {hex(ufs.fwver)}")
                    self.debug(f"UFS Blocksize:{hex(ufs.block_size)}")
                    try:
                        self.debug(f"UFS ID:       {ufs.cid[2:].decode('utf-8')}")
                    except:
                        pass
                    self.debug(f"UFS CID:      {hexlify(ufs.cid).decode('utf-8')}")
                    if self.config.hwparam is not None:
                        self.config.set_cid(ufs.cid)
                    self.debug(f"UFS LU0 Size: {hex(ufs.lu0_size)}")
                    self.debug(f"UFS LU1 Size: {hex(ufs.lu1_size)}")
                    self.debug(f"UFS LU2 Size: {hex(ufs.lu2_size)}")
    ...
    def upload(self):
        ...
        self.debug(f"Uploading xflash stage 1 from {os.path.basename(loader)}")
        if not os.path.exists(loader):
            self.debug(f"Couldn't find {loader}, aborting.")
            return False
        with open(loader, 'rb') as bootldr:
            ...
            if self.mtk.preloader.send_da(da1address, da1size, da1sig_len, da1):
                self.debug("Successfully uploaded stage 1, jumping ..")
                if self.mtk.preloader.jump_da(da1address):
                    ...
                    if sync != b"\xC0":
                        ...
                        if res == pack("<I", self.Cmd.SYNC_SIGNAL):
                            self.debug("Successfully received DA sync")
                            return True
                        else:
    ...
    def reinit(self, display=False):
        ...
        if speed == b"full-speed":
            self.debug("Reconnecting to preloader")
            ...
            while not self.mtk.port.cdc.connect():
                time.sleep(0.5)
            self.debug("Connected to preloader")
    ...
    def upload_da(self):
        if self.upload():
            ...
            if emmc_info is not None and emmc_info.user_size != 0:
                self.debug("DRAM config needed for : " + hexlify(emmc_info.cid[:8]).decode('utf-8'))
            else:
                ufs_info = self.get_ufs_info()
                if ufs_info is not None and ufs_info.block_size != 0:
                    self.debug("DRAM config needed for : " + hexlify(ufs_info.cid).decode('utf-8'))
            ...
            if connagent == b"brom":
                ...
                if self.daconfig.emi is None:
                    self.debug("No preloader given. Searching for preloader")
                    found = False
                    self.debug("Sending emi data ...")
                                ...
                                if emmc_info is not None:
                                    if emmc_info.cid[:8] in data:
                                        ...
                                        if not self.send_emi(self.daconfig.emi):
                                            ...
                                            found = True
                                            self.debug("Detected working preloader: " + preloader)
                                            break
                                else:
                    ...
                    if not self.send_emi(self.daconfig.emi):
                        ...
                        self.debug("Sending emi data succeeded.")
            elif connagent == b"preloader":
                stage = 1
            if stage == 1:
                self.debug("Uploading stage 2...")
                with open(self.daconfig.loader, 'rb') as bootldr:
                    ...
                    if loaded:
                        self.debug("Successfully uploaded stage 2")
                        self.reinit(True)
                        ...
                        if daextdata is not None:
                            ...
                            if self.boot_to(at_address=0x68000000, da=daextdata):
                                ...
                                if status == 0x0 and unpack("<I", ret)[0] == 0xA1A2A3A4:
                                    self.debug("DA Extensions successfully added")
                                    self.daext = True
                            if not self.daext:
