...
class xflashext(metaclass=LogBase):
    ...
    def custom_rpmb_init(self):
        ...
        if self.config.chipconfig.meid_addr:
            ...
            if meid != b"\x00"*16:
                ...
                self.debug("Generating sej rpmbkey...")
                self.setotp(hwc)
        ...
        if self.cmd(cmd):
            ...
            if int.from_bytes(derivedrpmb[:4],'little') != 0xff:
                status = self.status()
                if status == 0:
                    self.debug("Derived rpmb key:" + hexlify(derivedrpmb).decode('utf-8'))
                    return True
            ...
            with open(filename, "wb") as wf:
                for sector in range(sectors):
                    ...
                    if data == b"":
                        ...
                        return False
                    wf.write(data)
            self.debug("Done reading rpmb to " + filename)
            return True
        return False
                ...
                with open(filename, "rb") as rf:
                    for sector in range(sectors):
                        ...
                        if not self.custom_rpmb_write(sector=sector, data=rf.read(0x100), ufs=ufs):
                            ...
                            return False
                self.debug(f"Done reading writing {filename} to rpmb")
                return True
        return False
            ...
            if sectors > 0:
                for sector in range(sectors):
                    ...
                    if not self.custom_rpmb_write(sector=sector, data=b"\x00" * 0x100, ufs=ufs):
                        ...
                        return False
                self.debug(f"Done erasing rpmb")
                return True
        return False
    ...
    def generate_keys(self):
        ...
        if self.config.chipconfig.dxcc_base is not None:
            self.debug("Generating dxcc rpmbkey...")
            rpmbkey = hwc.aes_hwcrypt(btype="dxcc", mode="rpmb")
            self.debug("Generating dxcc fdekey...")
            fdekey = hwc.aes_hwcrypt(btype="dxcc", mode="fde")
            self.debug("Generating dxcc rpmbkey2...")
            rpmb2key = hwc.aes_hwcrypt(btype="dxcc", mode="rpmb2")
            self.debug("Generating dxcc km key...")
            ikey = hwc.aes_hwcrypt(btype="dxcc", mode="itrustee")
            # self.debug("Generating dxcc platkey + provkey key...")
            # platkey, provkey = hwc.aes_hwcrypt(btype="dxcc", mode="prov")
            # self.debug("Provkey     : " + hexlify(provkey).decode('utf-8'))
            # self.debug("Platkey     : " + hexlify(platkey).decode('utf-8'))
            if rpmbkey is not None:
                self.debug("RPMB        : " + hexlify(rpmbkey).decode('utf-8'))
            ...
            if rpmb2key is not None:
                self.debug("RPMB2       : " + hexlify(rpmb2key).decode('utf-8'))
            ...
            if self.config.chipconfig.prov_addr:
                ...
                self.debug("PROV        : " + hexlify(provkey).decode('utf-8'))
            ...
            if hrid is not None:
                self.debug("HRID        : " + hexlify(hrid).decode('utf-8'))
