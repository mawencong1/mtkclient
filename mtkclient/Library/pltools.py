...
class PLTools(metaclass=LogBase):
    ...
    def runpayload(self, filename, offset=0, ack=0xA1A2A3A4, addr=None, dontack=False):
        ...
        try:
            with open(filename, "rb") as rf:
                rf.seek(offset)
                payload = rf.read()
                self.debug(f"Loading payload from {os.path.basename(filename)}, {hex(len(payload))} bytes")
        except FileNotFoundError:
            self.debug(f"Couldn't open {filename} for reading.")
            return False
        ...
        if ptype == "amonet":
            self.debug("Amonet Run")
            if self.payload(payload, addr, ptype):
                if dontack:
                    return True
                result = self.usbread(4)
                if result == pack(">I", ack):
                    self.debug("Successfully sent payload: " + filename)
                    return True
                self.debug("Error, payload answered instead: " + hexlify(result).decode('utf-8'))
                return False
            else:
                self.error("Error on sending payload: " + filename)
            return True
        elif ptype in ("kamakiri", "kamakiri2"):
            self.debug("Kamakiri / DA Run")
            ...
            self.debug("Carbonara is best served at local restaurants.")
        else:
            self.debug("Kamakiri / DA Run")
                    ...
                    if "preloader" in rf.name:
                        ...
                        if ack == b"\xC0\xC0\xC0\xC0":
                            with open("preloader.bin", 'wb') as wf:
                                ...
                                self.debug("Preloader dumped as: " + "preloader.bin")
                                return True
                    else:
                        with open("out.bin", 'wb') as wf:
                            ...
                            for pos in range(0, 0x20000, 64):
                                wf.write(self.mtk.port.usbread(64))
                            self.debug("Bootrom dumped as: " + "out.bin")
                            return True
                self.debug("Error, payload answered instead: " + hexlify(result).decode('utf-8'))
                return False
            else:
    ...
    def crasher(self, mtk, enforcecrash: bool = False):
        ...
        if enforcecrash or self.config.meid is None:
            self.debug("We're not in bootrom, trying to crash da...")
    ...
    def run_dump_brom(self, filename, btype, loader="generic_dump_payload.bin"):
        ...
        if btype == "amonet":
            ...
            self.debug("Kamakiri2")
            if self.mtk.config.chipconfig.send_ptr[0] is None:
                self.debug("Unknown chipset, please run \"brute\" command and send the brom as an issue on github")
                return False
            if self.runpayload(filename=pfilename, ack=0xC1C2C3C4, offset=0):
                if self.kama.dump_brom(filename):
                    self.debug("Dumped as: " + filename)
            ...
            self.debug("Available ptypes are: amonet, kamakiri, kamakiri2, hashimoto, carbonara")
