...
class DA_handler(metaclass=LogBase):
    ...
    def configure_da(self, mtk, preloader):
        ...
        if mtk.port.cdc.connected is None or not mtk.port.cdc.connected:
            ...
            if mtk.port.cdc.connected and os.path.exists(".state"):
                ...
                mtk.daloader.upload_da(preloader=preloader)
                return mtk
        if mtk.config.target_config is None:
            self.debug("Please disconnect, start mtkclient and reconnect.")
            return None
        if mtk.config.target_config["daa"]:
            mtk = mtk.bypass_security()
            self.mtk = mtk
            self.debug("Device is protected.")
            if mtk is not None:
                if mtk.config.is_brom:
                    self.debug("Device is in BROM mode. Trying to dump preloader.")
                    if preloader is None:
                        preloader = self.dump_preloader_ram()
        else:
            self.debug("Device is unprotected.")
            # if not mtk.config.is_brom:
            #   self.mtk.preloader.reset_to_brom()
            if mtk.config.is_brom:
                self.debug("Device is in BROM-Mode. Bypassing security.")
                ...
                self.debug("Device is in Preloader-Mode :(")
    ...
    def da_read_memory(self, partitionname, parttype, filename, offset, length, packetsize=0x20):
        ...
        if rpartition is not None:
            self.debug(f"Dumping partition memory\"{rpartition.name}\"")
            return self.mtk.daloader.readflash(addr=rpartition.sector * self.mtk.daloader.daconfig.pagesize + offset,
                                        length=length,
                                        filename="", parttype=parttype, packetsize=0x100000)
        return None
    ...
    def da_read(self, partitionname, parttype, filename):
        ...
        if parttype == "user" or parttype is None:
            i = 0
            countDump = 0
            self.debug("Requesting available partitions ....")
            gpttable = self.mtk.daloader.get_partition_data(parttype=parttype)
                ...
                if partition == "gpt":
                    ...
                    if rpartition is not None:
                        ...
                        if self.mtk.daloader.readflash(addr=rpartition.sector * self.mtk.daloader.daconfig.pagesize,
                                                       ...
                                                       filename=partfilename, parttype=parttype):
                            self.debug(f"Dumped sector {str(rpartition.sector)} with sector count " +
                                      f"{str(rpartition.sectors)} as {partfilename}.")
                            countDump += 1
                        else:
                            self.debug(f"Failed to dump sector {str(rpartition.sector)} with sector count " +
                                      f"{str(rpartition.sectors)} as {partfilename}.")
                            countDump += 1
                    else:
                        self.error(f"Error: Couldn't detect partition: {partition}\nAvailable partitions:")
                        for rpartition in gpttable:
                            self.debug(rpartition.name)
            if countDump > 1 and countDump == len(filenames):
                self.debug(f"All partitions were dumped")
            elif countDump > 1 and countDump != len(filenames):
                self.debug(f"Failed to dump some partitions")
        else:
            i = 0
        ...
        if guid_gpt is None:
            ...
            for partition in guid_gpt.partentries:
                ...
                if self.mtk.daloader.readflash(addr=partition.sector * self.mtk.daloader.daconfig.pagesize,
                    ...
                    countGPT += 1
                    self.debug(f"Dumped partition {str(partition.name)} as {str(filename)}.")
                else:
                    countGPT -= 1
            ...
            if countGPT == partitionsForRead:
                self.debug(f"All Dumped partitions success.")
            else:
    ...
    def da_wl(self, parttype: str, directory: str):
        ...
        if parttype == "user" or parttype is None:
            ...
            for partfilename in filenames:
                ...
                i += 1
                if partition == "gpt":
                    self.debug(f"Writing partition {partition}")
            ...
            for partfilename in filenames:
                ...
                partition = os.path.basename(partfilename)
                partition = os.path.splitext(partition)[0]
                self.debug(f"Writing filename {partfilename}")
                if self.mtk.daloader.writeflash(addr=pos, length=size, filename=partfilename,
    ...
    def da_efuse(self):
        if self.mtk.config.chipconfig.efuse_addr is not None:
            ...
            for idx in range(len(efuseconfig.efuses)):
                ...
                self.debug(f"EFuse Idx {hex(idx)}: {data.hex()}")
    ...
    def da_peek(self, addr: int, length: int, filename: str):
        ...
        if filename is not None:
            wf.close()
            self.debug(f"Successfully wrote data from {hex(addr)}, length {hex(length)} to {filename}")
        else:
            self.debug(
    ...
    def handle_da_cmds(self, mtk, cmd: str, args):
        ...
        if cmd == "gpt":
            ...
            parttype = args.parttype
            filename = args.filename
            offset = args.offset & 0xfffffc00
            slice_offset = args.offset - offset
            length = args.length + slice_offset
            return self.da_read_memory(partitionname=partitionname, parttype=parttype, filename=filename, offset=offset, length=length, packetsize=0x100000)[slice_offset:]
        elif cmd == "rl":
            directory = args.directory
            parttype = args.parttype
            if args.skip:
            ...
            if not os.path.exists(filename):
                ...
                self.close()
                return
            self.debug(f"Writing offset {hex(start)} with length {hex(length)}")
