...
class DALegacy(metaclass=LogBase):
    ...
    def set_stage2_config(self, hwcode):
        ...
        if toread == 4 and buffer == pack(">I", 0xBC3):
            ...
            pdram[1] = draminfo[:9]
            self.debug("DRAM config needed for : " + hexlify(draminfo).decode('utf-8'))
            ...
            if self.usbread(4) == pack(">I", 0xBC4):  # Nand_Status
                ...
                self.debug("Reading dram nand info ...")
                nand_ids = []
                ...
                if self.daconfig.emi is not None:  # toDo
                    ...
                    if ret == self.Rsp.ACK:
                        ...
                        if self.daconfig.emiver in [0x10, 0x14, 0x15]:
                            ...
                            self.debug("RAM-Length: " + hex(dramlength))
                            self.usbwrite(self.Rsp.ACK)
                            ...
                            self.debug("RAM-Info: " + hexlify(info).decode('utf-8'))
                        ...
                        self.debug(f"M_EXT_RAM_TYPE : {hex(m_ext_ram_type)}")
                        m_ext_ram_chip_select = self.usbread(1)[0]  # 0x00 CS_0
                        self.debug(f"M_EXT_RAM_CHIP_SELECT : {hex(m_ext_ram_chip_select)}")
                        m_ext_ram_size = unpack(">Q", self.usbread(8))[0]  # 0x80000000
                        self.debug(f"M_EXT_RAM_SIZE : {hex(m_ext_ram_size)}")
    ...
    def upload(self):
        ...
        self.debug(f"Uploading legacy stage 1 from {os.path.basename(loader)}")
        with open(loader, 'rb') as bootldr:
            ...
            if self.mtk.preloader.send_da(da1address, da1size, da1sig_len, da1):
                if self.mtk.preloader.jump_da(da1address):
                    ...
                    if sync != b"\xC0":
                        ...
                        self.debug("Got loader sync !")
                else:
                    return False
            else:
                return False
            self.debug("Reading nand info")
            ...
            self.debug("Setting stage 2 config ...")
                    ...
                    if self.read_flash_info():
                        ...
                        if speed[0] == 0:  # 1 = USB High Speed, 2= USB Ultra high speed
                            ...
                            while not self.mtk.port.cdc.connect():
                                self.debug("Waiting for reconnection")
                                time.sleep(0.5)
    ...
    def upload_da(self):
        self.debug("Uploading legacy da...")
        if self.upload():
            self.debug(self.flashconfig)
    ...
    def brom_send(self, dasetup, dadata, stage, packetsize=0x1000):
        ...
        with open("/home/mwc/Desktop/2.bin", "wb") as f:
            f.write(dadata)
        ...
        if buffer == self.Rsp.ACK:
            ...
            buffer = self.usbread(1)
            if buffer == self.Rsp.ACK:
                self.debug(f"Successfully uploaded stage {stage}")
    ...
    def readflash(self, addr, length, filename, parttype=None, display=True, packetsize = 0x100000):
        self.mtk.daloader.progress.clear()
        length, parttype = self.get_parttype(length, parttype)
        self.check_usb_cmd()
        # packetsize = 0x0
