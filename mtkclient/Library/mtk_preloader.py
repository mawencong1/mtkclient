...
class Preloader(metaclass=LogBase):
    ...
    def init(self, maxtries=None, display=True):
        ...
        if not display:
            self.debug("Status: Waiting for PreLoader VCOM, please reconnect mobile to brom mode")
            self.config.set_gui_status(self.config.tr("Status: Waiting for connection"))
        else:
            self.debug("Status: Waiting for PreLoader VCOM, please connect mobile")
            self.config.set_gui_status(self.config.tr("Status: Waiting for connection"))
        res = False
        tries = 0
        ...
        if self.display:
            self.debug("\tCPU:\t\t\t" + cpu + "(" + self.config.chipconfig.description + ")")
            self.config.cpu = cpu.replace("/", "_")
            self.debug("\tHW version:\t\t" + hex(self.config.hwver))
            self.debug("\tWDT:\t\t\t" + hex(self.config.chipconfig.watchdog))
            self.debug("\tUart:\t\t\t" + hex(self.config.chipconfig.uart))
            self.debug("\tBrom payload addr:\t" + hex(self.config.chipconfig.brom_payload_addr))
            self.debug("\tDA payload addr:\t" + hex(self.config.chipconfig.da_payload_addr))
            if self.config.chipconfig.cqdma_base is not None:
                self.debug("\tCQ_DMA addr:\t\t" + hex(self.config.chipconfig.cqdma_base))
            self.debug("\tVar1:\t\t\t" + hex(self.config.chipconfig.var1))

        if not skipwdt:
            if self.display:
                self.debug("Disabling Watchdog...")
            self.setreg_disablewatchdogtimer(self.config.hwcode)  # D4
        if self.display:
            self.debug("HW code:\t\t\t" + hex(self.config.hwcode))
        self.config.target_config = self.get_target_config(self.display)
        self.debug("Get Target info")
        self.get_blver()
        self.get_bromver()
        ...
        if self.display:
            self.debug("\tHW subcode:\t\t" + hex(self.config.hwsubcode))
            self.debug("\tHW Ver:\t\t\t" + hex(self.config.hwver))
            self.debug("\tSW Ver:\t\t\t" + hex(self.config.swver))
        meid = self.get_meid()
        if meid is not None:
            self.config.set_meid(meid)
            if self.display:
                self.debug("ME_ID:\t\t\t" + hexlify(meid).decode('utf-8').upper())
            if readsocid or self.config.chipconfig.socid_addr:
                ...
                if self.display:
                    if socid != b"":
                        self.debug("SOC_ID:\t\t\t" + hexlify(socid).decode('utf-8').upper())
        return True
    ...
    def get_target_config(self, display=True):
        if self.echo(self.Cmd.GET_TARGET_CONFIG.value):
            ...
            if display:
                self.debug(f"Target config:\t\t{hex(target_config)}")
                self.debug(f"\tSBC enabled:\t\t{sbc}")
                self.debug(f"\tSLA enabled:\t\t{sla}")
                self.debug(f"\tDAA enabled:\t\t{daa}")
                self.debug(f"\tSWJTAG enabled:\t\t{swjtag}")
                self.debug(f"\tEPP_PARAM at 0x600 after EMMC_BOOT/SDMMC_BOOT:\t{epp}")
                self.debug(f"\tRoot cert required:\t{cert}")
                self.debug(f"\tMem read auth:\t\t{memread}")
                self.debug(f"\tMem write auth:\t\t{memwrite}")
                self.debug(f"\tCmd 0xC8 blocked:\t{cmd_c8}")
    ...
    def jump_da(self, addr):
        self.debug(f"Jumping to {hex(addr)}")
        ...
        if self.echo(self.Cmd.JUMP_DA.value):
            ...
            if resaddr == addr:
                try:
                    ...
                    return False
                if status == 0:
                    self.debug(f"Jumping to {hex(addr)}: ok.")
    ...
    def upload_data(self, data, gen_chksum):
        ...
        pos = 0
        with open("/home/mwc/Desktop/1.bin", "wb") as f:
            f.write(data)
